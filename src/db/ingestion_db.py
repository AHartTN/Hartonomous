...
UPDATED: Now uses FractalAtomizer for composition deduplication.
"""
...
class IngestionDB:
    ...
    def __init__(self, pool: AsyncConnectionPool):
        ...
        self._fractal_atomizer = None  # Lazy init on first use
        ...
        async with self.pool.connection() as conn:
            async with conn.cursor() as cur:
                for seq_idx, comp_id in enumerate(component_atom_ids):
                    ...
                    """,
                        (parent_atom_id, comp_id, seq_idx, json.dumps(metadata or {})),
                    )
