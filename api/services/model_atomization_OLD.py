...
def parse_tensor_name(tensor_name: str) -> Dict[str, int]:
    ...
    Common patterns:
    - blk.5.attn_q.weight -> layer=5, head=0
    - model.layers.5.self_attn.q_proj.weight -> layer=5, head=0
    - transformer.h.5.attn.c_attn.weight -> layer=5, head=0
    
    Args:
        tensor_name: Full tensor name from GGUF model
        
    Returns:
        Dict with 'layer_idx' and 'head_idx' (defaults to 0 if not found)
    """
    layer_idx = 0
    head_idx = 0
    
    # Pattern 1: blk.N or block.N
    match = re.search(r'blk\.(\d+)|block\.(\d+)', tensor_name, re.IGNORECASE)
    if match:
        layer_idx = int(match.group(1) or match.group(2))
    
    # Pattern 2: layers.N or layer.N
    match = re.search(r'layers?\.(\d+)', tensor_name, re.IGNORECASE)
    if match:
        layer_idx = int(match.group(1))
    
    # Pattern 3: h.N (transformer heads)
    match = re.search(r'\.h\.(\d+)\.', tensor_name)
    if match:
        layer_idx = int(match.group(1))
    
    # Head index (if present - rare in GGUF)
    match = re.search(r'head[._]?(\d+)', tensor_name, re.IGNORECASE)
    if match:
        head_idx = int(match.group(1))
    
    return {'layer_idx': layer_idx, 'head_idx': head_idx}
...
class GGUFAtomizer(BaseAtomizer):
    ...
    async def _atomize_gguf_file(
        ...
        # LAZY LOADING: Pass tensor references, not preloaded data
        # Load tensors on-demand within worker tasks to avoid OOM with large models
        logger.info(f"Processing {len(tensors_to_process)} tensors with lazy loading...")
        import time
        
        # Build lightweight tensor metadata list (no weight data yet)
        tensor_data_list = []
        for idx, tensor in enumerate(tensors_to_process):
            tensor_data_list.append({
                'tensor': tensor,  # Just reference, weights loaded on-demand
                'tensor_idx': idx,
                'reader': reader  # Include reader for model config access
            })
        ...
        max_workers = settings.pool_num_workers  # Configurable worker count
        ...
        async def _process_tensor(worker_conn, tensor_idx: int, tensor_data: dict, pool: Optional[AsyncConnectionPool]):
            """Inner function to process a single tensor with lazy weight loading."""
            tensor = tensor_data['tensor']
            reader = tensor_data.get('reader')  # Get reader for model config
            
            # LAZY LOAD: Load tensor weights on-demand (not preloaded)
            logger.info(
                f"Processing tensor {tensor_idx+1}/{len(reader.tensors)}: {tensor.name} {tensor.shape}"
            )
            logger.debug(f"  Task {tensor_idx}: Loading tensor weights on-demand...")
            weights = tensor.data.flatten()  # Load just this tensor's data
            total_weights = len(weights)
            logger.debug(f"  Task {tensor_idx}: Loaded {total_weights:,} weights")
            ...
            # DEFER composition creation - store without lock (each task gets its own entry)
            # Use thread-safe list append (GIL makes this safe for simple appends)
            ...
            # Process weights (lazy-loaded above)
            logger.info(f"  Task {tensor_idx}: Processing {total_weights:,} weights...")
            ...
            tensor_metadata_dict = None
            if settings.enable_weight_spatial_positioning:
                # Parse tensor name to extract layer/head indices
                name_metadata = parse_tensor_name(tensor.name)
                
                # Build model config from reader fields (if available)
                model_config = {}
                try:
                    # Try to get model architecture info from reader
                    # These field names are from GGUF spec
                    if hasattr(reader, 'fields'):
                        model_config = {
                            'block_count': reader.fields.get('llama.block_count', {}).get('parts', [32])[0] if 'llama.block_count' in reader.fields else 32,
                            'attention.head_count': reader.fields.get('llama.attention.head_count', {}).get('parts', [32])[0] if 'llama.attention.head_count' in reader.fields else 32,
                        }
                except Exception as e:
                    logger.debug(f"Could not extract model config from reader: {e}")
                    model_config = {'block_count': 32, 'attention.head_count': 32}  # Defaults
            ...
            if component_ids:
                ...
                await self._create_composition_batch_arrays(
                    worker_conn, tensor_atom_id, component_ids, sequence_indices, pool
                )
    ...
    async def _atomize_weight_batch(
        ...
        Args:
            ...
            weights: List of weight values to atomize
            tensor_metadata: Optional dict with 'name', 'shape', 'layer_idx', 'head_idx' for spatial positioning
            weight_indices: Optional list of indices (positions) corresponding to weights for spatial encoding
        """
        import time
        ...
        # Check cache first (optimized: batch lookup)
        logger.info(f"    Checking cache for {len(unique_weights):,} unique weights...")
        cache_start = time.time()
        
        # Quick atomic cache lookup - get cached and uncached in one pass
        cached_map = {}
        uncached_weights = []
        
        async with self.cache_lock:
            for w in unique_weights:
                atom_id = self.cache.get(w)
                if atom_id is not None:
                    cached_map[w] = atom_id
                else:
                    uncached_weights.append(w)
        ...
        if uncached_weights:
            ...
            if tensor_metadata and settings.enable_weight_spatial_positioning:
                ...
                logger.info(f"    → Computing spatial keys for {len(uncached_weights):,} weights...")
                
                # Extract tensor metadata
                tensor_shape = tensor_metadata.get('shape', [])
                layer_idx = tensor_metadata.get('layer_idx', 0)
                head_idx = tensor_metadata.get('head_idx', 0)
                model_config = tensor_metadata.get('model_config', {})
                
                # Build index mapping if indices provided
                weight_to_index = {}
                if weight_indices:
                    for i, w in enumerate(weights):
                        if w not in weight_to_index:
                            weight_to_index[w] = weight_indices[i]
                
                for idx, weight in enumerate(uncached_weights):
                    # Get position index for this weight
                    position_idx = weight_to_index.get(weight, idx) if weight_indices else idx
                    
                    # Compute row/col from flat index (assume 2D tensor)
                    if len(tensor_shape) >= 2:
                        cols = tensor_shape[-1]
                        row_idx = position_idx // cols
                        col_idx = position_idx % cols
                    else:
                        row_idx = position_idx
                        col_idx = 0
                    
                    # Calculate spatial key
            ...
            # Pre-compute metadata JSON once (shared by all weights)
            metadata_dict = {"modality": "weight"}
            if tensor_metadata:
                metadata_dict.update({
                    "tensor_name": tensor_metadata.get('name', ''),
                    "layer_idx": tensor_metadata.get('layer_idx', 0),
                    "head_idx": tensor_metadata.get('head_idx', 0)
                })
            metadata_json = json.dumps(metadata_dict)
            
            # Vectorized hash computation using NumPy
            import numpy as np
            rows = []
            
            # Batch process in chunks to show progress and manage memory
            HASH_BATCH_SIZE = 10000
            num_batches = (len(uncached_weights) + HASH_BATCH_SIZE - 1) // HASH_BATCH_SIZE
            
            for batch_idx in range(num_batches):
                start_idx = batch_idx * HASH_BATCH_SIZE
                end_idx = min((batch_idx + 1) * HASH_BATCH_SIZE, len(uncached_weights))
                batch_weights = uncached_weights[start_idx:end_idx]
                
                for local_idx, weight in enumerate(batch_weights):
                    global_idx = start_idx + local_idx
                    
                    # Convert weight to string once
                    weight_str = str(weight)
                    content_hash = hashlib.sha256(weight_str.encode('utf-8')).digest()
                    
                    # Include spatial key if computed
                    if spatial_keys:
                        rows.append((content_hash, weight_str, metadata_json, spatial_keys[global_idx]))
                    else:
                        rows.append((content_hash, weight_str, metadata_json))
                
                # Progress reporting - less frequent now
                if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == num_batches:
                    print(f"    → Hashed {end_idx}/{len(uncached_weights)} weights ({100*end_idx//len(uncached_weights)}%)", end='\r', flush=True)
            print()  # New line after progress
            
            # Bulk insert atoms using utility function
            copy_time, results = await bulk_insert_atoms(
                conn, rows, 
                include_spatial=bool(spatial_keys),
                chunk_size=5000,
                show_progress=True
            )
            ...
            # Build cache mappings OUTSIDE lock
            ...
            # Build complete cache update dict OUTSIDE lock (no lock contention)
            cache_updates = {}
            ...
            logger.info(f"    → Built {len(cache_updates):,} cache mappings ({cache_build_time:.2f}s)")
            
            # SINGLE atomic lock for batch cache update (no contention!)
            logger.info(f"    → Acquiring cache lock for batch update...")
            lock_start = time.time()
            async with self.cache_lock:
                self.cache.update(cache_updates)
                self.stats["atoms_created"] += len(cache_updates)
            lock_time = time.time() - lock_start
            logger.info(f"    → Cache updated in {lock_time:.4f}s (lock held for minimal time)")
    ...
    async def _create_composition_batch_arrays(
        ...
        """Batch create compositions using COPY with progress bar."""
        
        # Bulk insert compositions using utility function
        ...
        logger.info(f"    ✓ Composition COPY complete: {total_rows:,} rows in {copy_time:.2f}s ({total_rows/copy_time:,.0f} rows/s)")
    ...
    async def _create_composition_batch(
        ...
        """
        Batch create compositions with parallel execution for large datasets.
        
        Uses connection pool for concurrent batch processing to maximize throughput.
        """
        import time
        ...
        async def insert_batch(batch_idx: int, start: int, end: int):
            ...
            # Extract data for bulk operation
            ...
            async with pool.connection() as batch_conn:
                # Use utility function for clean COPY operation
                await bulk_insert_compositions(
                    ...
                    sequence_indices,
                    chunk_size=10000,
                    show_progress=False  # Disable individual progress bars for parallel batches
                )
            return batch_idx, end - start
        
        # Create batch tasks (process multiple batches concurrently)
        
        CONCURRENT_BATCHES = settings.pool_num_workers  # Run batches in parallel (configurable)
        tasks = []
