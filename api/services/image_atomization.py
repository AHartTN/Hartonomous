...
class ImageAtomizer:
    ...
    def __init__(self, patch_size: int = 16, dedupe_threshold: float = 0.01):
        ...
        self.pixel_cache: Dict[Tuple[int, int, int, int], int] = (
            {}
        )  # (R,G,B,A) → atom_id
    ...
    async def atomize_image(
        ...
        async with conn.cursor() as cur:
            ...
            await self._atomize_pixels(conn, img_rgba, patch_atoms, width, height)
    ...
    async def _atomize_pixel(
        ...
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                """,
                (color_hash, f"rgba_{r}_{g}_{b}_{a}", json.dumps(pixel_metadata)),
            )
