    ...
    if _embedding_model is None:
        try:
            ...
            if model_path.exists():
                ...
                logger.info(
                    f"Downloading embedding model ({model_name}) to {CACHE_DIR}..."
                )
...
def project_to_3d(
    ...
    if fit_references or _reference_embeddings is None:
        ...
        for _ in range(n_refs - 1):
            ...
            dists = np.sum(
                (embeddings[:, None, :] - embeddings[indices, :]) ** 2, axis=2
            )
    ...
    refs_norm = _reference_embeddings / (
        np.linalg.norm(_reference_embeddings, axis=1, keepdims=True) + 1e-8
    )
...
def get_embedding_stats() -> Dict[str, any]:
    ...
    return {
        ...
        "num_references": (
            len(_reference_embeddings) if _reference_embeddings is not None else 0
        ),
