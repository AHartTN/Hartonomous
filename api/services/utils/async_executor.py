...
async def run_cpu_batch(
    ...
    try:
        ...
        for func, args, kwargs in tasks:
            if kwargs:
                func_with_kwargs = partial(func, **kwargs)
                async_tasks.append(loop.run_in_executor(executor, func_with_kwargs, *args))
            else:
...
class AsyncExecutorContext:
    ...
    async def run_batch(self, tasks: List[Tuple[Callable, Tuple, dict]]) -> List[Any]:
        ...
        return await run_cpu_batch(tasks, max_workers=self.max_workers, executor=self.executor)
