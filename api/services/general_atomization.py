...
class AtomizationService:
    ...
    async def atomize_text(
        conn: AsyncConnection, text: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            async with conn.cursor() as cur:
                ...
                await cur.execute(
                    "SELECT atomize_text(%s, %s::jsonb);", (text, json.dumps(metadata))
                )

                result = await cur.fetchone()
                atom_ids = result[0] if result else []

                logger.info(f"Atomized text: {len(atom_ids)} atoms created")
    ...
    async def atomize_image(
        ...
        try:
            ...
            async with conn.cursor() as cur:
                ...
                return {"atom_count": atom_count, "root_atom_id": root_atom_id}
