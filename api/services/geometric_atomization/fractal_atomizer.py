...
class FractalAtomizer:
    ...
    def __init__(
        self, db_connection=None, coordinate_range: float = 1e6, bpe_crystallizer=None
    ):
        """
    ...
    def locate_composition(
        self, child_ids: List[int], strategy: str = "midpoint"
    ...
    async def get_or_create_primitive(self, value: bytes, metadata: dict = None) -> int:
        """
        ...
        atom_id = await query_one_returning(
            ...
            RETURNING atom_id
        """,
            (content_hash, value, *point_params, json.dumps(metadata or {})),
        )
    ...
    async def get_or_create_composition(
        ...
        row = await query_one(
            ...
            LIMIT 1
        """,
            (*point_params, 1e-6, *point_params),
    ...
    async def crystallize_sequence(
        self, sequence: List[bytes], greedy: bool = True, learn: bool = True
    ) -> List[int]:
        """
