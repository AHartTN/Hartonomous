...
class GeometricAtomizer:
    ...
    def __init__(self, db_connection=None, coordinate_range: float = 1e6):
        ...
        self.fractal = FractalAtomizer(
            db_connection=db_connection, coordinate_range=coordinate_range
        )
    ...
    async def atomize_text(self, text: str) -> str:
        ...
        wkt = await run_cpu_bound(
            self.builder.build_from_atoms, atom_values, self.locator
        )

        return wkt

    async def atomize_tensor(
        self, tensor: np.ndarray, chunk_size: int = 10000
    ) -> List[str]:
        """
        ...
        wkts = await run_cpu_batch_simple(
            build_chunk, chunks, max_workers=optimal_workers
        )

        return list(wkts)

    async def store_trajectory(self, name: str, wkt: str, metadata: dict = None) -> int:
        """
        ...
        for x, y, z, m in points:
            ...
            atom_id = await self.fractal.get_or_create_primitive(
                f"{x},{y},{z},{m}".encode("utf-8"), metadata={"coord": [x, y, z, m]}
            )
        ...
        composition_id = await self.fractal.get_or_create_composition(
            child_ids=atom_ids, canonical_text=name, metadata=meta, is_stable=True
        )

        return composition_id

    async def reconstruct_from_name(self, name: str, output_type: str = "bytes") -> Any:
        """
    ...
    async def _reconstruct_text_from_composition(
        self, composition_ids: List[int]
    ) -> str:
    ...
    async def _reconstruct_bytes_from_composition(
        self, composition_ids: List[int]
    ) -> bytes:
    ...
    async def ingest_model(
        self, model_state_dict: dict, model_name: str, chunk_size: int = 10000
    ) -> dict:
        """
    ...
    async def reconstruct_model(self, model_name: str) -> dict:
        """
