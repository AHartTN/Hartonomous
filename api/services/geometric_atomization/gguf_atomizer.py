...
from ..utils import query_one, execute_query
...
class GGUFAtomizer:
    ...
    async def atomize_model(
        ...
        if self.stats['total_processed'] > 0:
            self.stats['deduplication_ratio'] = (
                ...
                if self.stats['atoms_created'] > 0 else 1.0
            )
        ...
        logger.info(f"  Deduplication ratio: {self.stats.get('deduplication_ratio', 0):.1f}x")
    ...
    async def _atomize_tensor_as_trajectory(
        ...
        weight_bytes = [
            np.float32(w).tobytes() for w in non_sparse_weights
        ]
