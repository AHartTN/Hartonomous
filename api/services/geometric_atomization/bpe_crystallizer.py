...
class BPECrystallizer:
    ...
    def get_merge_candidates(self, top_k: int = 10) -> List[Tuple[Tuple[int, int], int]]:
        """
        ORIENT: Identify most frequent pairs.
    ...
    async def decide_and_mint(
        ...
        for pair, count in self.pair_counts.most_common():
            ...
            if auto_mint:
                ...
                logger.info(f"Minted composition {composition_id} for pair {pair} (freq={count}, ratio={frequency:.4f})")
