...
class QueryService:
    ...
    async def get_atom_by_id(
        conn: AsyncConnection, atom_id: int
    ...
    async def get_atom_lineage(
        conn: AsyncConnection, atom_id: int, max_depth: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        try:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                return {"nodes": nodes, "total_ancestors": len(nodes)}
    ...
    async def spatial_search(
        ...
        try:
            async with conn.cursor(row_factory=dict_row) as cur:
                ...
                logger.info(f"Spatial search for '{query}': {len(results)} results")
