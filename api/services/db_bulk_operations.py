...
class BulkCopyOperation:
    ...
    async def execute_binary(
        self, rows: List[Tuple], description: str = "COPY operation"
    ) -> float:
        """
        ...
        async with self.conn.cursor() as cur:
            async with cur.copy(self.copy_statement) as copy:
                if self.show_progress:
                    with tqdm(
                        total=self.total_rows, desc=description, unit="rows", ncols=100
                    ) as pbar:
            ...
            async with cur.copy(self.copy_statement) as copy:
                if self.show_progress:
                    with tqdm(
                        ...
                        await self._write_text_with_progress(
                            copy, rows, formatter, pbar
                        )
                else:
    ...
    async def _write_text_with_progress(
        self, copy, rows: List[Any], formatter: Callable, pbar
    ):
...
async def bulk_insert_atoms(
    ...
    if include_spatial:
        copy_stmt = (
            "COPY atom (content_hash, canonical_text, metadata, spatial_key) FROM STDIN"
        )
    else:
...
async def bulk_insert_compositions(
    ...
    elapsed = await operation.execute_text(
        rows, format_tsv, f"Inserting {len(rows):,} compositions"
    )
