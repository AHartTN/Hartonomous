...
public static class HilbertCurve4D
{
        ...
        int precision = DefaultPrecision)
    {
        ...
        Marshal.NativeMethods.HilbertEncode4D(x, y, z, m, precision, out resultHigh, out resultLow);
        ...
        int precision = DefaultPrecision)
    {
        ...
        try
        {
            ...
            Marshal.NativeMethods.HilbertDecode4D(high, low, precision, gcHandle.AddrOfPinnedObject());
    ...
    /// Includes overflow protection and max-range clamping.
    /// </summary>
        ...
        int precision = DefaultPrecision)
    {
        ...
        // FIX: Handle large radii (infinite search) by clamping to max coordinate range
        // instead of casting to uint (which overflows at 4B)
        long r = (long)System.Math.Ceiling(radius);
        long maxVal = (1L << precision) - 1;

        // Calculate bounds using long to prevent overflow
        ...
        // If radius covers the entire universe, return full range
        if (minX == 0 && maxX == maxVal && 
            minY == 0 && maxY == maxVal && 
            minZ == 0 && maxZ == maxVal && 
            minM == 0 && maxM == maxVal)
        {
            return (0, 0, ulong.MaxValue, ulong.MaxValue);
        }
