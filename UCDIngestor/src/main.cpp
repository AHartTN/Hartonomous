...
int main(int argc, char* argv[]) {
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] 
                  << " <ucd.all.flat.xml> <allkeys.txt> <confusables.txt> <emoji-sequences.txt> <emoji-zwj-sequences.txt>" 
                  << std::endl;
        return 1;
    }
    ...
    std::string emoji_path = argv[4];
    std::string emoji_zwj_path = argv[5];
    ...
    try {
        ...
        ingestor.run_gene_pool_ingestion(xml_path, allkeys_path, confusables_path, emoji_path, emoji_zwj_path);
