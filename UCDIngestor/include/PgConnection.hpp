...
public:
    ...
    std::string at(size_t row, size_t column) const override { 
        if (row >= m_result.size()) {
            throw std::out_of_range("Attempt to access out-of-range result row.");
        }
        // Cast column to int to avoid ambiguity
        return m_result[row][(int)column].c_str(); 
    }
    
    std::string at(size_t row, const std::string& column_name) const override { 
        if (row >= m_result.size()) {
            throw std::out_of_range("Attempt to access out-of-range result row.");
        }
...
public:
    ...
    void bulk_upsert(const std::string& table_name, const std::vector<std::map<std::string, std::string>>& data_list, const std::string& conflict_target, const std::vector<std::string>& update_columns) override;
    void bulk_stream_upsert(const std::string& table_name, const std::vector<std::map<std::string, std::string>>& data_list, const std::string& conflict_target, const std::vector<std::string>& update_columns) override;
};
