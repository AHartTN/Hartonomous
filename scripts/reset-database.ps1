...
Write-Host "`n? Resetting sequences..." -ForegroundColor Yellow

try {
    $resetSeqCmd = @"
ALTER SEQUENCE atom_atom_id_seq RESTART WITH 1;
ALTER SEQUENCE atom_composition_composition_id_seq RESTART WITH 1;
ALTER SEQUENCE spatial_region_region_id_seq RESTART WITH 1;
ALTER SEQUENCE relation_relation_id_seq RESTART WITH 1;
"@
    & psql -h $env:PGHOST -p $env:PGPORT -U $env:PGUSER -d $env:PGDATABASE -c $resetSeqCmd 2>&1 | Out-Null
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "? Sequences reset" -ForegroundColor Green
    }
    else {
        Write-Host "? Failed to reset sequences (may not exist yet)" -ForegroundColor Yellow
    }
}
catch {
    Write-Host "? Failed to reset sequences: $_" -ForegroundColor Yellow
}
