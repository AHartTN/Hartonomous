...
async def validate_text_ingestion(coordinator: IngestionCoordinator):
    ...
    test_file.write_text(
        "Hello, World! This is a test of the Hartonomous ingestion pipeline."
    )

    try:
        result = await coordinator.ingest_file(
            test_file, source_id="validation_text_001"
        )
...
"""
    ...
    try:
        result = await coordinator.ingest_file(
            test_file, source_id="validation_code_001"
        )
...
async def validate_batch_ingestion(coordinator: IngestionCoordinator):
    ...
    try:
        for i in range(5):
            ...
            test_file.write_text(
                f"Batch test file #{i} for Hartonomous pipeline validation."
            )
            test_files.append(
                {
                    "path": str(test_file),
...
async def validate_error_handling(coordinator: IngestionCoordinator):
    ...
    result = await coordinator.ingest_file(
        Path("non_existent_file.txt"), source_id="validation_error_001"
    )
    ...
    try:
        result = await coordinator.ingest_file(
            test_file, source_id="validation_error_002"
        )
...
async def main():
    ...
    logger.info("?" + "=" * 58 + "?")
    logger.info(
        "?" + " " * 10 + "HARTONOMOUS INGESTION PIPELINE VALIDATION" + " " * 6 + "?"
    )
    logger.info("?" + "=" * 58 + "?")
