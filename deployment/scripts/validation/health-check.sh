...
API_HOST=$(echo "$DEPLOYMENT_CONFIG" | jq -r '.api.host // "localhost"')
API_PORT=$(echo "$DEPLOYMENT_CONFIG" | jq -r '.api.port // 8000')
API_URL="http://${API_HOST}:${API_PORT}"

write_log "Checking API at: $API_URL" "INFO"
...
if RESPONSE=$(curl -s -f -m 10 "$API_URL/v1/health" 2>/dev/null); then
    STATUS=$(echo "$RESPONSE" | jq -r '.status // "unknown"')
    write_success "API Health: $STATUS"
    HEALTH_CHECKS+=("API_Health:PASS")
else
    write_log "API health check failed at $API_URL/v1/health" "ERROR"
...
if curl -s -f -m 10 "$API_URL/v1/health/database" >/dev/null 2>&1; then
...
if [[ "$NEO4J_ENABLED" == "true" ]]; then
    ...
    if curl -s -f -m 10 "$API_URL/v1/health/neo4j" >/dev/null 2>&1; then
...
if curl -s -f -m 10 "$API_URL/v1/metrics" >/dev/null 2>&1; then
    write_success "Metrics: Available"
    HEALTH_CHECKS+=("Metrics:PASS")
else
    write_log "Metrics endpoint not available (optional)" "INFO"
