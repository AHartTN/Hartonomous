        ...
        for (size_t f_start = 0; f_start < files.size(); f_start += CHUNK_SIZE) {
            ...
            for (size_t i = f_start; i < f_end; ++i) {
                ...
                for (const auto& sent : results[i - f_start].sents) {
                    std::vector<Service::ComputedComp> sc; for (const auto& tok : sent) sc.push_back(Service::compute_comp(tok.lemma, lookup));
            ...
            for (size_t i = 0; i < results.size(); ++i) {
                for (size_t si = 0; si < results[i].sents.size(); ++si) {
                    ...
                    for (size_t ti = 0; ti < sent.size(); ++ti) {
                        merge_comp(c_comps[ti], sent[ti].lemma, *batch);
                        auto it = g_cache.get_comp(sent[ti].lemma);
                        if (it) token_comps[sent[ti].id] = *it;
                    }
            ...
            if (processed % 500 == 0) std::cout << "  Processed " << processed << " files..." << std::endl;
        }
        flusher.wait_all();
