...
def test_landmark_projection():
    ...
    landmark_pos = projector.project_from_content(
        weights.flatten()[:100],  # Use subset
        modality="model",
        category="weight"
    )
    
    print(f"  ✅ Generated landmark position")
    print(f"  ✅ Position: x={landmark_pos.x:.3f}, y={landmark_pos.y:.3f}, z={landmark_pos.z:.3f}")
...
def test_end_to_end():
    ...
    for atom in atoms:
        for pos in landmark_positions:
            ...
            associations.append({
                'atom_id': atom.content_hash,
                'landmark': pos,
                'distance': dist
            })
    
    print(f"  ✅ Created {len(associations)} associations")
    print(f"  ✅ Average distance: {np.mean([a['distance'] for a in associations]):.3f}")
