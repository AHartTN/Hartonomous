...
async def test_gguf_ingestion():
    """Test ingesting first tensor from qwen3-coder:30b."""
    # Ollama blob path from `ollama show qwen3-coder:30b --modelfile`
    model_path = Path(r"D:\Models\blobs\sha256-1194192cf2a187eb02722edcc3f77b11d21f537048ce04b67ccf8ba78863006a")
    ...
    logger.info(f"Connecting to: {settings.pghost}:{settings.pgport}/{settings.pgdatabase}")
        ...
        async with pool.connection() as conn:
            async with conn.transaction():
                result = await atomizer.atomize_model(
                    ...
                    conn=conn,
                    max_tensors=1  # Just first tensor for testing
                )
