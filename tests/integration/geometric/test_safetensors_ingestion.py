...
class TestSafeTensorsLoading:
    ...
    async def test_load_safetensors_file(self, test_safetensors_path):
        ...
        found_patterns = [
            p for p in expected_patterns if any(p in k for k in tensors.keys())
        ]

        assert (
            len(found_patterns) > 0
        ), f"Should have typical embedding layers, found: {list(tensors.keys())[:5]}"
...
class TestDatabaseStorage:
    ...
    async def test_store_single_embedding(self, db_connection, clean_db):
        ...
        async with db_connection.cursor() as cur:
            await cur.execute(
                "SELECT name, metadata FROM atom_composition WHERE id = %s", (comp_id,)
            )
...
class TestFullModelIngestion:
    ...
    async def test_ingest_minilm_mock(self, db_connection, clean_db):
        ...
        mock_state_dict = {
            "embeddings.word_embeddings.weight": np.random.randn(100, 256).astype(
                np.float32
            ),
            "embeddings.position_embeddings.weight": np.random.randn(512, 256).astype(
                np.float32
            ),
            "encoder.layer.0.attention.self.query.weight": np.random.randn(
                256, 256
            ).astype(np.float32),
            "encoder.layer.0.attention.self.key.weight": np.random.randn(
                256, 256
            ).astype(np.float32),
...
class TestRealSafeTensorsIngestion:
    ...
    async def test_ingest_first_5_layers(
        self, db_connection, clean_db, test_safetensors_path
    ):
        ...
        # Ingest
        comp_ids = await atomizer.ingest_model(
            model_state_dict=first_5, model_name="test-minilm-partial", chunk_size=10000
        )

        # Verify
        assert len(comp_ids) >= len(
            first_5
        ), "Should create compositions (may be chunked)"
    ...
    async def test_query_stored_embeddings(
        self, db_connection, clean_db, test_safetensors_path
    ):
