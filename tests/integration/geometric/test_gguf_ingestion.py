...
class SimpleGGUFReader:
    ...
    def read_first_tensors(self, max_tensors: int = 5, max_values: int = 1000) -> Dict:
        ...
        mock_tensors = {
            "output.weight": (
                np.random.randn(32000, 2048).astype(np.float32)[:10, :10],
                "float32",
            ),
            "blk.0.attn_q.weight": (
                np.random.randn(2048, 2048).astype(np.float32)[:10, :10],
                "float32",
            ),
            "blk.0.attn_k.weight": (
                np.random.randn(2048, 2048).astype(np.float32)[:10, :10],
            ...
            "blk.0.ffn_gate.weight": (
                np.random.randn(5632, 2048).astype(np.float32)[:10, :10],
                "float32",
            ),
            "token_embd.weight": (
                np.random.randn(32000, 2048).astype(np.float32)[:10, :10],
...
class TestDatabaseIngestion:
    ...
    async def test_store_single_trajectory(self, db_connection, clean_db):
        ...
        comp_id = await atomizer.store_trajectory(
            name="test.weight", wkt=wkts[0], metadata={"shape": [5], "dtype": "float32"}
        )
...
class TestModelIngestion:
    ...
    async def test_reconstruct_mock_model(self, db_connection, clean_db):
        ...
        assert len(reconstructed) == len(
            original_state_dict
        ), "All layers should be reconstructed"

        for layer_name, original_tensor in original_state_dict.items():
            assert (
                layer_name in reconstructed
            ), f"Layer {layer_name} should be reconstructed"

            reconstructed_tensor = reconstructed[layer_name]
            assert (
                reconstructed_tensor.shape == original_tensor.shape
            ), "Shape should match"
...
class TestRealGGUFIngestion:
    ...
    async def test_ingest_first_5_tensors(
        self, db_connection, clean_db, test_gguf_path
    ):
