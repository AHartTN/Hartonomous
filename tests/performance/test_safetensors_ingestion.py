...
async def test_safetensors_ingestion():
    ...
    print(
        f"   Components: Model {'+ Config' if config_path else ''} {'+ Tokenizer' if tokenizer_path else ''}"
    )
    print()
    ...
    async with await get_db_connection() as conn:
        ...
        try:
            ...
            if result.get("weight_stats"):
                ...
                sparse_pct = (
                    (sparse_count / total_weights * 100) if total_weights > 0 else 0
                )
                processing_time = weights["processing_time_secs"]
                weights_per_sec = (
                    total_weights / processing_time if processing_time > 0 else 0
                )
                ...
                print(
                    f"   Sparse weights skipped: {sparse_count:,} ({sparse_pct:.2f}%)"
                )
                ...
                print(
                    f"   Memory savings: {(1 - 1/dedup_ratio)*100:.1f}%"
                    if dedup_ratio > 1
                    else "   Memory savings: 0%"
                )
                print()
