...
async def test_semantic_embeddings():
    ...
    test_tokens = [
        ...
        # Vehicles
        "car", "truck", "automobile", "vehicle", "bus",
        # Colors
        "red", "blue", "green", "yellow", "purple",
        # Actions
        "run", "jump", "walk", "sprint", "jog",
    ]
    ...
    print("\n" + "=" * 60)
    print("📈 Summary:")
    print(f"   Within-category avg: {np.mean([avg_animal_dist, avg_vehicle_dist, avg_action_dist]):.4f}")
    print(f"   Cross-category avg: {avg_cross_dist:.4f}")
    
    ratio = avg_cross_dist / np.mean([avg_animal_dist, avg_vehicle_dist, avg_action_dist])
