...
public class MechanisticInterpretabilityService
{
    ...
    public async Task<NeuronInterpretationResult> InterpretNeuronAsync(Guid componentId, string userId, NeuronInterpretationRequest request)
    {
        ...
        try
        {
            ...
            // Get existing neuron interpretations through repository
            var existingInterpretations = await _componentRepository.GetComponentInterpretationsAsync(componentId, userId);
            var interpretation = existingInterpretations.FirstOrDefault();
    ...
    private async Task StorePatternsAsync(Guid modelId, List<NeuralPattern> patterns, string userId)
    {
        foreach (var pattern in patterns)
        {
            ...
            // Also store pattern relationships in knowledge graph
            try
            {
                await _knowledgeGraphRepository.CreateComponentRelationshipAsync(
                    pattern.ComponentId,
                    pattern.ComponentId, // Self-reference for pattern
                    $"HAS_PATTERN_{pattern.Type.ToUpper()}",
                    userId);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to create pattern relationship in knowledge graph for component {ComponentId}", pattern.ComponentId);
            }
        }
