...
public class AgentDistillationService
{
    ...
    {
        var component = await _componentRepository.GetByIdAsync(modelComponentId, userId);
        ...
        var usageComponents = await _componentRepository.GetAgentUsagesAsync(modelComponentId, userId);
        var avgUsageRelevance = usageComponents.Where(ac => ac.IsActive).Any()
            ? usageComponents.Where(ac => ac.IsActive).Average(ac => ac.ComponentWeight) : 0.0;

        return (baseRelevance + capabilityBoost + avgUsageRelevance) / 3.0;
    }
